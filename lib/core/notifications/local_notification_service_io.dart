...
/// Native (mobile/desktop) notification backend over
/// `flutter_local_notifications`. This file is only compiled on platforms that
/// have `dart:io` — the web build uses `local_notification_service_web.dart`
/// instead, because the plugin transitively pulls `dart:io`/`dart:ffi` in via
/// its Linux backend and would break `flutter build web`.
