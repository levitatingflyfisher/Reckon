...
Future<void> goldenAtSizes(
  ...
  ThemeData? theme,
  // Some screens never "settle": a CircularProgressIndicator (or fl_chart's
  // entry animation) keeps the frame scheduler busy forever, so
  // pumpAndSettle would time out. For those, set [settle] false and pump a
  // fixed [pumpDuration] enough for post-frame data loads + animation to land.
  bool settle = true,
  ...
  for (final MapEntry<String, Size> size in sizes.entries) {
    for (final double scale in textScales) {
      ...
      if (settle) {
        ...
        // Drain post-frame callbacks (data loads) then advance the clock far
        // enough for entry animations, without waiting for a settle that
        // never comes.
        await tester.pump();
